...
namespace KSGGames {
	...
	namespace
	{
		Square Chess_Board_P[8][8];
		Square Test_Board_P[8][8];
		std::string Types_P[9];
	}
	...
	public:
		ChessPrac(void)
		{
			...
			for (int i = 0; i < 8; i++)
			{
				...
				for (int j = 0; j < 8; j++)
				{
					if (j < 2) Chess_Board_P[i][j].SetTeam("black");
					else if (j > 5) Chess_Board_P[i][j].SetTeam("white");
					else Chess_Board_P[i][j].SetTeam("neutral");

					if (j == 1 || j == 6) Chess_Board_P[i][j].SetType("Pawn");
					else if (j < 2 || j > 5) Chess_Board_P[i][j].SetType(Types_P[i]);
				}
			}
	...
	private: System::Void boxClick(System::Object^  sender, System::EventArgs^  e)
	{
		if (ableToSwap)
		{
			...
			copyArr(Chess_Board_P, Test_Board_P);

			if ( firstCheckUp(Test_Board_P, xFrom, yFrom, xTo, yTo, Test_Move) )
			{
				if ( elPass(Test_Board_P, xFrom, yFrom, xTo, yTo, Test_Move) && !checkAfter(Test_Board_P, Test_Move))
				{
					copyArr(Test_Board_P, Chess_Board_P);
					whitesMove = Test_Move;
					clearMoved(Chess_Board_P, Test_Move);
				...
				else if ( castling(Test_Board_P, xFrom, yFrom, xTo, yTo, Test_Move) )
				{
					copyArr(Test_Board_P, Chess_Board_P);
					whitesMove = Test_Move;
					clearMoved(Chess_Board_P, Test_Move);
				...
				else if (otherMove(Test_Board_P, xFrom, yFrom, xTo, yTo, Test_Move) && !checkAfter(Test_Board_P, Test_Move))
				{
					copyArr(Test_Board_P, Chess_Board_P);
					whitesMove = Test_Move;
					clearMoved(Chess_Board_P, Test_Move);
				...
				if (endPawn(Chess_Board_P, whitesMove))
				{
					System::ComponentModel::ComponentResourceManager^  resources = (gcnew System::ComponentModel::ComponentResourceManager(ChessPrac::typeid));
	...
	private: System::Void choosePawn(System::Object^  sender, System::EventArgs^  e)
	{
		...
		std::string loc = changeEndPawn(Chess_Board_P, type);
		x = loc[0] - '0';
		y = loc[1] - '0';
		...
		String^ Types_P = msclr::interop::marshal_as<String^>(picture);
		Board[x, y]->Image = (cli::safe_cast<System::Drawing::Image^>(resources->GetObject(Types_P)));
